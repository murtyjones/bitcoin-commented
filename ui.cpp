...
void CSendDialog::OnButtonSend(wxCommandEvent& event)
{
    CWalletTx wtx;
    /**
     * Get the recipient's address. This can be either the
     * bitcoin address or the IP address of the recipient.
     */
    string strAddress = (string)m_textCtrlAddress->GetValue();

    /**
     * Get the amount of money to send to the recipient.
     * 
     * TODO is this in Satoshis?
     */
    int64 nValue = 0;
    if (!ParseMoney(m_textCtrlAmount->GetValue(), nValue) || nValue <= 0)
    {
        /**
         * If there was an error parsing the amount (e.g. if 
         * it was malformed), show an error and do nothing.
         */
    ...
    {
        // If the user doesn't have enough coins, show an error and do nothing
    ...
    {
        /**
         * If the user doesn't have enough money once the transaction
         * fee is included, show an error and do nothing.
         */
    ...
    /**
     * Attempt to parse the recipient's address into a hash 160 of a
     * bitcoin address. This will fail if the address is an IP address
     * or just generally invalid.
     */
    uint160 hash160;
    ...
    if (fBitcoinAddress)
    {
        ...
        // Payment successfully sent, show a message:
